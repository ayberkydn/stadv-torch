...
def apply_flow(img_batch, flow_batch):
    ...
    added = repeat(
        torch.tensor([        
            ...
            dtype=torch.long, 
            device=device,
        ), 'ind c -> b h w ind c', b=BATCH_SIZE, h=H, w=W)


    sampled_pixel_indices = repeat(
        torch.floor(sampling_grid).long(), 
        'b h w c -> b h w 4 c', 
    ) + added 


    sampled_pixel_distances = torch.abs(sampled_pixel_indices - repeat(sampling_grid, 'b h w c -> b h w c2 c', c2=4)) 
    ...
    sampled_pixels_flat = torch.gather(
        input=img_batch_flat, 
        index=sampled_pixel_indices_reduced_flat, 
        dim=-1
    )
    sampled_pixels = rearrange(sampled_pixels_flat, 'b c (h w four) -> b c h w four', h=H, w=W, four = 4)
    
    sampled_pixels_weighted = sampled_pixels * repeat(sampled_pixel_weights, 'b h w four -> b c h w four', c = 3)
    sampled_pixels_weighted_sum = reduce(sampled_pixels_weighted, 'b c h w four -> b c h w', reduction='sum')
